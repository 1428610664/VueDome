<template>
	<transition name="move">
		<div class="myapply-wrapper b3">
			<x-header :left-options="{backText: ''}">{{headTitle}}</x-header>
							...
							<div class="item-info" @click="onItemClick(item)">
								...
								<div class="info">
									...
									<div class="sundry clear">
										<div class="launch hzline1">发起：<span class="c1">{{item.ApplyName}}</span></div> <span class="status" :class="_parseState(item.State)">{{item.StateName}}</span><span class="time fr c2">{{_parseTime(item.CreatTime)}}</span></div>
								</div>
				...
				<div v-transfer-dom>
				    <popup v-model="selCustomTime" position="bottom"  :hide-on-blur="false">
				        ...
				        <group label-width="5em" label-margin-right="1em" label-align="right">
				        		<x-input title="流程主题：" v-model="title"></x-input>
				        		<x-input title="单据编码：" v-model="code"></x-input>
				        		<selector v-show="$route.query.id != 2" v-model="pageId" title="流程类型：" :options="pageIdArray"></selector>
...
<script>
	...
	import { XHeader, Tab, TabItem, XInput, Selector, TransferDom, Popup, Group, Datetime } from 'vux'
	import $ from 'jquery'
	import utils from 'common/js/utils'
	import GetWeekDate from 'common/js/WeekDate'
	import * as apply from 'api/pending'
	...
	export default {
		...
		data() {
			return {
				...
				selCustomTime: false,
				customStartDate: utils.getCurrentDate(),
				customEndDate: '',
				pageIdArray: [],
		...
		methods: {
			init(){
				...
				this.headTitle = id == 0 ? "我的申请" : id == 2 ? this.$route.query.title : "我已审批"
				setTimeout(() => {
					this.getList()
					if(id != 2)this.getFlowPage()
				}, 800)
			},
			pullRefresh(){
				setTimeout(() => {
			...
			onItemClick(item) {
				...
				this.$router.push({path: path + '/outsourcemanage',query:{flowid: item.FlowId, flowpageid: item.PageId,flowmodelid: item.FlowModelId, title: item.PageName}})
			},
			getList(mark) {
				if(mark){
					this.pageNo = 1
				...
				if(this.currentIndex == 4){
					...
					reData.code = this.code
					if(id != 2)reData.pageId = this.pageId
				}
				if(id == 2)reData.pageId = this.$route.query.pageId
				
				apply[id == 0 ? "getMyapply" : id == 2 ? "getMyFlow" : "getMyapproval"](reData).then((data) => {
					if(mark){
						this.isLoadEnd = false
				...
				getFlowPage().then((data) => {
					if(data.Result){
						...
						for(let i = 0, ilen = data.Data.length; i< ilen; i++){
							this.pageIdArray.push({key: data.Data[i].PageId, value: data.Data[i].PageName})
						}
					}
				},(error) => {
		...
		watch: {
			'$route' (to, from) {
				if(to.path == "/me/myapply" || to.path == "/me/my-process/myapply" ){
					if(!this.isInit){
						this.isInit = true
						this.init()
					}
