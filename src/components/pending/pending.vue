		...
		<div v-transfer-dom>
		    <popup v-model="selCustomTime" position="bottom"  :hide-on-blur="false">
		        ...
		        <group label-width="5em" label-margin-right="1em" label-align="right">
	        		...
	        		<selector v-model="pageId" title="流程类型：" :options="pageIdArray"></selector>
...
<script>
	...
	import { Tab, TabItem, XInput, Selector, TransferDom, Popup, Group, Datetime } from 'vux'
	import $ from 'jquery'
		...
		methods: {
			...
			getTaskList(mark) {
				...
				if(this.currentIndex == 4){
					...
					reData.pageId = this.pageId
					reData.code = this.code
				}
				
				getTaskList(reData).then((data) => {
					if(mark){
						this.isLoadEnd = false
