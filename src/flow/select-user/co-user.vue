...
<script>
	...
	export default {
		...
		methods: {
			show(title, type, i){ // type: 0协办 1指派
				if(type == 0){
					markVal = "协办"
				}else if(type == 1){
					markVal = "指派"
			...
			confirm(){
				if(utils.isEmpty(this.dept[0])){
					Toast('请选择'+markVal+'部门')
					return
				}
				if(utils.isEmpty(this.staff[0])){
					Toast('请选择'+markVal+'职员')
					return
				}
				let index = this._findIndex(staffList, this.staff[0], true)
				this.$emit("confirm",{id: staffList[index].PerArchivePk, type: this.type}, this.eventIndex)
				this.hide()
			},
			// 获取所有部门
			getAlldept(){
